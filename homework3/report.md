...
1. **動態性**：
   - 使用鏈結串列允許多項式項數動態增加或減少，無需預先分配固定空間，特別適合處理項數不定的輸入場景。

2. **簡單實現**：
   - `Node` 結構直接包含係數 (`coef`) 和指數 (`exp`) 以及指針 (`link`)，提供直觀的方式管理多項式項，減少額外資料結構的設計複雜性。

3. **效能考量**：
   - 雖然鏈結串列的節點分配和釋放可能帶來記憶體管理開銷，但其靈活性在小規模多項式運算中表現優異。
   - 乘法中的合併邏輯透過指針調整節點，雖然效率受巢狀遍歷影響，但實現簡單，適合用於學習鏈結串列操作。
